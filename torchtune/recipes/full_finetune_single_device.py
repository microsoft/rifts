...
def focal_loss(logits, labels, gamma=2.0, alpha=0.25):
    """
    Compute focal loss between logits and labels.

    Args:
        logits: Predicted logits from the model (shape: [b, num_classes, s]).
        labels: True labels (shape: [b, s]).
...
class FullFinetuneRecipeSingleDevice(FTRecipeInterface):
    ...
    def validation_loop(self, dataset) -> None:
        special_tokens = [
            self._tokenizer.encode(f"<|reserved_special_token_{i}|>")[1]
            for i in [2, 3, 4, 5]
        ]
    ...
    def fix_untrained_tokens(self, eps=1e-16):
        embedding_matrix = self._model.tok_embeddings.weight.data
        lm_head_matrix = self._model.output.weight.data
    ...
    def train(self) -> None:
        ...
        # Initialize tokens count and running loss (for gradient accumulation)
        ...
        for curr_epoch in range(self.epochs_run, self.total_epochs):
            print("=====================================================")
            print(f"Starting epoch {curr_epoch} of {self.total_epochs}")
            ...
            for idx, batch in enumerate(self._dataloader):
                ...
                # Randomly select from 0th axis of tokens tensor for debug logging every 100 steps
                if idx % 100 == 0:
                    rand_select = torch.randint(0, tokens.shape[0], (1,))
                    print(tokens.shape)
                    print(labels.shape)
                    rand_tokens = tokens[rand_select][0]
                    rand_labels = labels[rand_select][0]
                    print("TOKENS")
                    print([self._tokenizer.decode([x]) for x in rand_tokens])
                    print(rand_tokens)
                    print("LABELS")
                    print(
                        [
                            self._tokenizer.decode([x])
                            for x in rand_labels
                            if x != self._loss_fn.ignore_index
                        ]
                    )
                    print(rand_labels)

                # Get the attention mask and position ids from the dataset if available.
                ...
                # Identify special tokens in the labels (converted to float for multiplication)
                is_special_token = torch.isin(labels, special_token_ids).float()

                # Compute the token-wise loss
                loss = self._loss_fn(logits, labels)

                # Upweight loss for special tokens by a factor of 2.
                # Approach 2, Option 1: separate special and non-special tokens and average only nonzero losses.
                loss_special = loss * is_special_token * 2.0
                loss_non_special = loss * (1 - is_special_token)
                loss_special = loss_special[loss_special != 0]
                loss_non_special = loss_non_special[loss_non_special != 0]
                loss = loss_special.mean() + loss_non_special.mean()

                # Free logits to reduce peak memory usage
                del logits
                ...
                # Step with optimizer based on gradient accumulation steps
